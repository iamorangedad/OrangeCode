...
def main():
    ...
    parser = argparse.ArgumentParser(description="No-RAG Coding Agent with Scheduler")
    ...
    try:
        if args.config:
            ...
            console.print(
                Panel(
                    f"[bold cyan]Agent Configuration[/bold cyan]\n\n"
                    f"Ollama Host: [green]{os.getenv('OLLAMA_HOST', 'http://10.0.0.55:11434')}[/green]\n"
        ...
        if args.query:
            # Single query mode
            asyncio.run(scheduler.process_message(args.query))
        else:
            # Interactive mode
            console = Console()
            console.print(
                Panel(
                    "[bold cyan]No-RAG Coding Agent[/bold cyan]\n\n"
                    "[yellow]Commands:[/yellow]\n"
                    "• [green]query[/green] - Ask a question\n"
            ...
            while True:
                try:
                    ...
                    console.print("\n[yellow]Use ESC to cancel operations[/yellow]")
                except EOFError:
                    break
